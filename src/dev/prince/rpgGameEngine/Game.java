...
public class Game {
	...
	public void start() {
		...
		if (JOptionPane.showConfirmDialog(null, this,
				"Do You Want To Run The Server?", 1) == 0) {
			server = new GameServer(handler);
			handler.getPlayer().setUsername(
					JOptionPane.showInputDialog(this, "Enter Username:"));
			GameServer.serverName = handler.getPlayer().getUsername();
			if (new File("res/multiplayerData/"
					+ handler.getPlayer().getUsername() + ".server").exists()) {
				String password = JOptionPane.showInputDialog(this,
						"Enter Password Of The Associated Server: ");
				password = password.replaceAll("\\s", "");
				String passCode = Utils.loadFileAsString("res/multiplayerData/"
				...
				if (password.equals(passCode)) {
					...
					JOptionPane.showMessageDialog(null,
							"WRONG PASSWORD..EXECUTION TERMINATED!   "
									+ passCode + "|");
					System.exit(1);
				}
			} else {
				Formatter f = null;
				try {
					f = new Formatter("res/multiplayerData/"
							+ handler.getPlayer().getUsername() + ".server");
				} catch (FileNotFoundException e) {
					e.printStackTrace();
				}
				String passCode = JOptionPane.showInputDialog(this,
						"Set Password Of The Associated Server: ");
				f.format(passCode + " ");
				f.close();
				server.start();
			}
			...
			if (JOptionPane.showConfirmDialog(null, this,
					"Do You Want To Join Server", 1) == 0) {
				Packet00Login login;
				...
				handler.getPlayer().setUsername(
						JOptionPane.showInputDialog(this, "Enter Username:"));
				login = new Packet00Login(handler.getPlayer().getUsername());// We
																				// will
																				// get
																				// x
																				// and
	...
	public void render() {
		 ...
		 * GL11.glClearColor(0, 0, 0, 0); Color.white.bind();
		 * keyManager.render();
		 * 
		 * if(State.getState() !=null){ State.getState().render(); }
		 * 
		 * TextArea.renderTextArea(handler.getWidth()-200,
		 * 33-25,"FPS: "+Frames+" UPS: "+Updates, Color.black);
		 * TextArea.renderTextArea(handler.getWidth()-200, 33,"Vsync: "+vsync,
		 * Color.black);
		 * 
		...
		GL11.glBindTexture(GL11.GL_TEXTURE_2D, 0); // unlink textures because if
													...
													// fail
		glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, Renderer.framebufferID);
		...
		TextArea.renderTextArea(handler.getWidth() - 200, 33 - 25, "FPS: "
				+ Frames + " UPS: " + Updates, Color.black);
		TextArea.renderTextArea(handler.getWidth() - 200, 33,
				"Vsync: " + vsync, Color.black);
	...
	public void disconnectMP() {
		Packet01Disconnect disconnect = new Packet01Disconnect(
				handler.getPlayerX(), handler.getPlayerY(), handler.getPlayer()
						.getUsername());
